...
class TestMigrations(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.test_db_url = "postgresql://postgres:postgres@localhost:5432/migration_test"
    ...
    def test_migration_rollback(self):
        ...
        self.assertEqual(current_version, previous_version,
    ...
    def test_preserve_data_on_migration(self):
        ...
        with self.Session() as session:
            ...
            session.execute(
                sa.text(
                    f"INSERT INTO {SCHEMA_NAME}.users (username, email, password_hash, is_active) "
        ...
        # Check that schema exists but data is gone (as expected after complete rollback)
        with self.Session() as session:
            ...
            result = session.execute(
                sa.text(
                    ...
                    f"WHERE table_schema = '{SCHEMA_NAME}' AND table_name = 'users'"
                    ")"
                )
            )
            table_exists = result.scalar()
            self.assertTrue(table_exists, "Users table does not exist after migration")
    ...
    def test_schema_matches_models(self):
        ...
        test_db_url2 = "postgresql://postgres:postgres@localhost:5432/model_test"
        self._create_test_database("postgresql://postgres:postgres@localhost:5432/postgres", "model_test")
        ...
        try:
            ...
            with self.engine.connect() as conn1, engine2.connect() as conn2:
                ...
                self.assertEqual(tables1, tables2, 
                               f"Tables don't match. Migration: {tables1}, Models: {tables2}")
            ...
            # Drop the test database
            self._drop_test_database("postgresql://postgres:postgres@localhost:5432/postgres", "model_test")
