...
# Setup logger
logger = setup_logger("migration_notifier")
...
def send_slack_notification(webhook_url, message, title=None, level="error"):
    ...
    try:
        ...
        # Prepare the command details for logging
        request_details = f"POST to {webhook_url}"
        ...
        if response.status_code == 200:
            log_command_execution(logger, "Slack API", "Slack notification sent successfully", 0)
            return True
        else:
            error_msg = f"Failed to send Slack notification: {response.status_code}, {response.text}"
            log_command_execution(logger, request_details, error_msg, 1)
            return False
            
    except Exception as e:
        log_command_execution(logger, "Slack API", f"Error sending Slack notification: {str(e)}", 1)
        return False
...
<body>
  ...
  <div style="margin-top: 30px; font-size: 11px; color: #999; font-style: italic;">
    This is an automated message from the Ultimate Marketing Team Migration System.
        ...
        # Log the SMTP connection attempt
        smtp_server_info = f"{smtp_config['server']}:{smtp_config['port']}"
        log_command_execution(logger, f"SMTP Connect", f"Connecting to {smtp_server_info}", 0)
        ...
        with smtplib.SMTP(smtp_config["server"], smtp_config["port"]) as server:
            if smtp_config.get("username") and smtp_config.get("password"):
                ...
                log_command_execution(logger, "SMTP Login", f"Logged in as {smtp_config['username']}", 0)
            server.send_message(msg)
        
        recipients_str = ', '.join(recipients)
        log_command_execution(logger, "SMTP Send", f"Email notification sent to {recipients_str}", 0)
        return True
        
    except Exception as e:
        error_msg = f"Error sending email notification: {str(e)}"
        log_command_execution(logger, "SMTP Error", error_msg, 1)
        return False
